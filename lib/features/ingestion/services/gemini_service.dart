...
import '../../recipes/models/nutrition_model.dart';
...
7. Estimate nutritional values per serving (based on defaultServings). All
   values are doubles. sodium is in mg, all others in grams (protein/fat/carbs/
   fiber/sugar/saturatedFat) or kcal (calories).
  ...
  RecipeModel _parseJson(String rawJson, String uid) {
    ...
    if (nutritionMap != null) {
      nutrition = NutritionModel(
        calories: (nutritionMap['calories'] as num?)?.toDouble() ?? 0,
        protein: (nutritionMap['protein'] as num?)?.toDouble() ?? 0,
        fat: (nutritionMap['fat'] as num?)?.toDouble() ?? 0,
        carbs: (nutritionMap['carbs'] as num?)?.toDouble() ?? 0,
        fiber: (nutritionMap['fiber'] as num?)?.toDouble() ?? 0,
        sugar: (nutritionMap['sugar'] as num?)?.toDouble() ?? 0,
        sodium: (nutritionMap['sodium'] as num?)?.toDouble() ?? 0,
        saturatedFat: (nutritionMap['saturatedFat'] as num?)?.toDouble() ?? 0,
      );
    }
