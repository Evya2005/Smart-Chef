...
import '../../recipes/providers/nutrition_refresh_provider.dart';
      ...
      body: ListView(
        children: [
          ...
          ListTile(
            ...
            title: const Text('עדכון ערכים תזונתיים'),
            subtitle: const Text(
                'מחשב ערכים תזונתיים לכל מנה באמצעות AI'),
            trailing: _NutritionRefreshTrailing(
                status: nutritionStatus, ref: ref),
          ),
          const Divider(),
          if (isAdmin) ...[
            Padding(
...
class _ApifyKeyTile extends ConsumerStatefulWidget {
  @override
  ConsumerState<_ApifyKeyTile> createState() => _ApifyKeyTileState();
}

class _ApifyKeyTileState extends ConsumerState<_ApifyKeyTile> {
  bool _helpExpanded = false;

  @override
  Widget build(BuildContext context) {
    final currentKey = ref.watch(apifyApiKeyProvider).asData?.value;
    final hasKey = ref.watch(hasApifyApiKeyProvider);
    ...
    return Column(
      ...
      children: [
        ListTile(
          ...
          subtitle: Text(
            hasKey
                ? 'ייבוא מפייסבוק/אינסטגרם פעיל'
                : 'נדרש לייבוא מתכונים מפייסבוק ואינסטגרם',
          ),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
        ...
        if (_helpExpanded)
          const Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _HelpStep(number: 1, text: 'גש לאתר apify.com וצור חשבון חינמי'),
                _HelpStep(
                    number: 2,
                    text: 'לחץ על שם המשתמש → Settings → Integrations'),
                _HelpStep(
                    number: 3,
                    text:
                        'תחת "Personal API tokens" לחץ "Create new token"'),
                _HelpStep(number: 4, text: 'העתק את המפתח והדבק כאן'),
                _HelpStep(
                    number: 5,
                    text: 'החשבון החינמי כולל \$5 קרדיט לחודש'),
              ],
            ),
          ),
      ],
  ...
  Future<void> _clearKey(BuildContext context) async {
    ...
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('הסר מפתח Apify?'),
        content: const Text('ייבוא מרשתות חברתיות לא יהיה זמין.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('ביטול'),
...
class _NutritionRefreshTrailing extends StatelessWidget {
  const _NutritionRefreshTrailing(
      {required this.status, required this.ref});

  final NutritionRefreshStatus status;
  final WidgetRef ref;

  Future<void> _run(BuildContext context) async {
    if (!await requireAiKey(context, ref)) return;
    ref.read(nutritionRefreshProvider.notifier).runNow();
  }

  @override
  Widget build(BuildContext context) {
    switch (status) {
      case NutritionRefreshStatus.running:
        return Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            const SizedBox(
            ...
            const Gap(8),
            Text('מעדכן...', style: Theme.of(context).textTheme.bodySmall),
          ],
        );
      case NutritionRefreshStatus.error:
        return Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.error_outline,
