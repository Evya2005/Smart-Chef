...
class _IngredientRowState extends State<_IngredientRow> {
  ...
  void initState() {
    ...
    _quantityCtrl = TextEditingController(
      text: widget.ingredient.quantity == 0
          ? ''
  ...
  void didUpdateWidget(_IngredientRow old) {
    ...
    if (widget.ingredient.name != _nameCtrl.text) {
      _nameCtrl.text = widget.ingredient.name;
    }
    final qText = widget.ingredient.quantity == 0
        ? ''
        : widget.ingredient.quantity.toString();
    if (qText != _quantityCtrl.text) {
      _quantityCtrl.text = qText;
    }
  }
...
class _StepRowState extends State<_StepRow> {
  ...
  void initState() {
    ...
    _instructionCtrl = TextEditingController(text: widget.step.instruction);
  }

  @override
  void didUpdateWidget(_StepRow old) {
    super.didUpdateWidget(old);
    if (widget.step.instruction != _instructionCtrl.text) {
      _instructionCtrl.text = widget.step.instruction;
    }
    final hasTimer = widget.step.timerSeconds != null;
    if (hasTimer != _showTimer) {
      setState(() => _showTimer = hasTimer);
    }
  }
