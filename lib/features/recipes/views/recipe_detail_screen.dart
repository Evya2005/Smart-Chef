...
class _RecipeDetailScreenState extends ConsumerState<RecipeDetailScreen> {
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    if (!isCooking) ...[
                      ...
                      const Gap(16),
                      NutritionPanel(recipeId: recipeId),
                      const Gap(8),
                    ],
                    Row(
                      children: [
  ...
  Future<void> _shareOnWhatsApp(recipe) async {
    final authorName = FirebaseAuth.instance.currentUser?.displayName;
    const svc = RecipeCardService();
    final card = svc.generateCard(recipe, authorName: authorName);
    final encoded = Uri.encodeComponent(card);
